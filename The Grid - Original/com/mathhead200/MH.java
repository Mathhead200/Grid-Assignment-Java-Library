...
{
	/**
	 * rounds <code>x</code> to <code>n</code> decimal places
	 * @param x - number
	 * @param n - precision
	 * @return the rounded double
	 */
	public static double round(double x, int n) {
		double e = Math.pow(10, n);
	...
	public static boolean isIn(int[] arr, int o) {
		for( long ele : arr ) {
			if( ele == o )
				return true;
		}
	...
	public static <T> void shuffle(java.util.List<T> list) {
		...
		for( int i = 0; i < list.size(); i++ )
			list.add( 0, list.remove( i + rand.nextInt(list.size() - i) ) );
	}

	public static String cypher(String input, String key) {
		char[] output = input.toCharArray();
		for( int z = 0; z < input.length() && z < key.length(); z++ ) {
			int i = (int)input.charAt(z),
				k = (int)key.charAt(z);
			output[z] = (char)(i ^ k);
		}
	...
	public static void encrypt(File input, File output, String key) throws IOException {
	...
	public static String encryptString(String input, String key) {
		...
		for( int i = 0; i < input.length(); i++ )
			str += cypher( "" + input.charAt(i), "" + key.charAt(i % key.length()) );
		return str;
	}
}
